...
draft: false
tags: ["documentation", "compiler", "hot-reloading"]
categories: []
...
1. **The compiler is running as a daemon**: Rather than being invoked and having to start from scratch, the compiler is running continuously, and 
   watches 
...
4. **Function hot-reloading**: To apply the code changes to the running program, we need to inject it into the running program. For interpreted or bytecode 
   based environments this should be fairly simple. For *native* code it seems a bit more complex, but should be doable with a trampoline mechanism or a 
...
The last part might require some support from the program itself, e.g. a way to snapshot and load the program state or the inputs. For terminal programs this 
might 
...
3. **Less waiting**: Even without all the fancy integration, having the hard work of compilation already done means that as soon as I want to 
   execute the program, it is already ready to go, thanks to the *literal* Ahead-Of-Time compilation.
...
1. **Complexity**: A query driven compiler is more complex than a traditional compiler.
2. **Correctness**: This approach requires caching, which has a reputation for causing issues, e.g. around staleness.
3. **Non-determinism**: This approach falls flat if there is any non-deterministic behaviour in the compiler, tests or program itself. Ideally I probably 
   want to avoid that. 
...
1. Is it viable to implement this approach in an understandable, fast and safe way?
